...
<table>
<tr>
<td width="50%"><img src="./examples/trailpaint-map.jpg" alt="作品範例 1" /></td>
<td width="50%"><img src="./examples/trailpaint-map-01.jpg" alt="作品範例 2" /></td>
</tr>
</table>
...
- **風格濾鏡**：原始 / 素描
- **即時預覽 + 構圖微調**：方向鍵平移、縮放
- **🔗 分享連結**：URL hash 編碼整個專案，收件人打開就是你的地圖（可選縮短成 TinyURL）
...
展示精選故事地圖，台灣宣教士腳蹤（馬偕、巴克禮、蘭大衛、馬雅各、彭蒙惠）是首批收錄。橫向 tabs 切換不同人物路線，LINE/FB 分享自動顯示 OG 縮圖。
...
### 匯出
**統一 ExportWizard**（圖片 / 備份 / 互通三 tab）· 多比例裁切即時預覽 · 3 種邊框 + 2 種濾鏡 · 統計 overlay · URL hash 分享連結 + TinyURL 縮址 · AI 提示詞 4 種風格 · Player 嵌入碼 · GeoJSON / KML 純地理結構
...
| Nominatim | 地點搜尋與反向地理編碼 | [nominatim.org](https://nominatim.openstreetmap.org) |
...
零網路環境可下載 [`trailpaint.html`](trailpaint.html) 單檔，瀏覽器直接開。

> ⚠️ 離線版不含線上地圖、GPX 匯入、海拔查詢、照片 EXIF、KML/GeoJSON 匯入等需要網路或額外依賴的功能。iOS 無法直接開啟本機 HTML，請使用 online 版。
