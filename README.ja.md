...
- **PWA 対応**：スマホのホーム画面にインストールしてネイティブアプリのように使える
...
<table>
<tr>
<td width="50%"><img src="./examples/trailpaint-map.jpg" alt="サンプル地図 1" /></td>
<td width="50%"><img src="./examples/trailpaint-map-01.jpg" alt="サンプル地図 2" /></td>
</tr>
</table>
...
- 地名浮現：Nominatim 逆ジオコーディングでスポット名を自動補完
...
- **🔗 共有リンク**：URL hash にプロジェクト全体を符号化、受信者は開くだけで地図を見られる（オプションで TinyURL 短縮可）
...
厳選ストーリー地図を掲載。台湾宣教師の足跡（馬偕・バークレー・ランズボロー・マクスウェル・ドリス・ブルハム）が第一弾。横タブで人物切り替え、LINE/FB 共有時に OG サムネイル自動表示。
...
### エクスポート
**統一 ExportWizard**（画像 / バックアップ / 互換の 3 tab）· マルチ比率クロップのリアルタイムプレビュー · 3 種フレーム + 2 種フィルター · 統計 overlay · URL hash 共有リンク + TinyURL 短縮 · AI プロンプト 4 スタイル · Player 埋め込みコード · GeoJSON / KML 純粋な地理構造
...
| Nominatim | 地点検索 + 逆ジオコーディング | [nominatim.org](https://nominatim.openstreetmap.org) |
...
完全にオフライン環境で使いたい場合、[`trailpaint.html`](trailpaint.html) 単一ファイルをダウンロードしてブラウザで直接開けます。

> ⚠️ オフライン版はオンライン地図・GPX インポート・標高照会・写真 EXIF・KML/GeoJSON インポートなど、ネットワークや追加依存を必要とする機能を含みません。iOS ではローカル HTML を直接開けないため、オンライン版（trailpaint.org）をご利用ください。
